...
exports.getInvoiceById = async (invoiceId, userId) => {
  const { rows } = await db.query(
    `SELECT * FROM invoices 
     WHERE id = $1 AND users_id = $2 AND deleted_at IS NULL`,
    [invoiceId, userId]
  );
...
exports.createInvoice = async (userId, data) => {
  ...
  const { rows } = await db.query(
    `INSERT INTO invoices (users_id, clients_id, invoice_number, invoice_date, due_date, total_amount, remaining_balance, status, currency, notes)
...
exports.searchInvoices = async (userId, filters) => {
  const { status, clientId, fromDate, toDate, minAmount, maxAmount } = filters;
